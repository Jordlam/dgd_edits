...
def get_feature(x, y, view, gaussians, pipeline, background, scaling_modifier, override_color, d_xyz, d_rotation, d_scaling, patch=None):
    ...
    # Here we take the (average) feature value of a patch for each image
    # i.e. if selecting a patch of the cookie (where the cookie is in that patch in all frames)
    #   this would return the feature value of the cookie shape [384]
    if patch is None:
